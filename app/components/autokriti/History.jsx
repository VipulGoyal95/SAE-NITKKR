...
const History = () => {
  ...
  const bind = useGesture({
    onDrag: ({ movement: [x], direction: [xDir], velocity: [xVel], cancel }) => {
      ...
      if (Math.abs(x) > 80 || Math.abs(xVel) > 0.5) {
        if (xDir > 0) {
  ...
  return (
    <div ref={componentRef} className="min-h-screen text-white px-16 max-[1025px]:px-8 max-[700px]:px-8 max-[500px]:px-4 max-[560px]:mx-auto overflow-hidden max-[1025px]:mt-8 max-[430px]:min-h-[80vh]" {...bind()}>
      <motion.h1 
      ...
      {mounted && hasAnimated && (
        <>
          {focusPosition === 0 && (
            ...
            >
              This September, gear up as North India’s biggest automotive takes a giant leap - bringing AI/ML into the mix.<br/>

              From roaring machines to mind-bending knowledge, from hands-on projects to a campus buzzing with energy—this year’s Autokriti is set to be bigger, smarter, and crazier than ever! It’s more than just a workshop. It’s an automotive fest.
            </motion.div>
          )}
          {focusPosition === 1 && (
            <motion.div
